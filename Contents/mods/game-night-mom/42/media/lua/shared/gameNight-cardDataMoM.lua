...
--- Pure indexing over MOM.sets/MOM.preBuiltSets - safe to run on both client and server.
--- Expansion add-ons (Desert Dreams, Inventor Legacy, etc.) add their own MOM.sets[...] and
--- MOM.preBuiltSets[...] entries from their own shared-scoped files before this runs, so every
--- installed expansion ends up visible on both sides once OnGameBoot fires.
function MOM.buildIndexes()
...
Events.OnGameBoot.Add(MOM.buildIndexes)


function MOM.rollDomain(rarity)
...
function MOM.weighedProbability(outcomesAndWeights)
    local totalWeight = 0
...
function MOM.rollCardOfParticularType(rarity, type)
    local cardPool = MOM.colorCodedRarity[rarity][type]
    return (cardPool[ZombRand(#cardPool)+1])
end


function MOM.rollCard(rarity, setID)
    ...
    local rollDomain = MOM.rollDomain(rarity)
    if rollDomain then
...
function MOM.spawnRandomCard(zombie)
    local rare = zombie and 1 or 2
    local common = zombie and 7 or 5
    local rarity = MOM.weighedProbability({ Common=common, Uncommon=3, Rare=rare})
    local card = MOM.rollCard(rarity)
    return card
end


function MOM.unpackBooster(cards, set)
    -- 11 common, 3 uncommon, 1 rare -- 15

    for i=1, 11 do
        local card = MOM.rollCard("Common", set)
        table.insert(cards, card)
    end

    for i=1, 3 do
        local card = MOM.rollCard("Uncommon", set)
        table.insert(cards, card)
    end

    for i=1, 1 do
        local card = MOM.rollCard("Rare", set)
        table.insert(cards, card)
    end
...
function MOM.buildDeck(archetype)
    ...
    archetype = archetype or MOM.weighedProbability(
    ...
    for i=1, artifactGoal do
        local rarity = MOM.weighedProbability({ Uncommon = 3, Rare = 1})
        --[[DEBUG]] rarities[rarity] = rarities[rarity]+1
        local card = MOM.rollCardOfParticularType(rarity, "Relics")
        table.insert(cards, card)
    end
    ...
    for i=1, DomainGoal do
        ...
        local card = MOM.rollCardOfParticularType("Domain", color.." Domain")
        table.insert(cards, card)
    end
    ...
    for i=1, remainingCount do
        local color = colors[ZombRand(#colors)+1]
        local rarity = MOM.weighedProbability({ Common = 11, Uncommon = 3, Rare = 1})
        --[[DEBUG]] rarities[rarity] = rarities[rarity]+1
        local card = MOM.rollCardOfParticularType(rarity, color)
        table.insert(cards, card)
    end
