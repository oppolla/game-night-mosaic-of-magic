...
--- This expansion file is required to be in /shared/ to work - not /client/. It must be visible
--- to the server so that booster packs and starter kits can resolve this set at craft time, not
--- just whenever a client happens to view them.
...
--- If your expansion registers its own sealed-pack item types (Base.momStarterKit_yourSet, etc.),
--- do it with a shared-safe require, not through the client-only gameNight-applyItemDetails.lua:
--- local gamePieceHandler = require("gameNight-gamePieceHandler.lua")
--- gamePieceHandler.registerSpecial("Base.yourStarterKitType", { hideUI = true, })
